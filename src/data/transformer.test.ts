...
const mockTimeRange = {
  ...
  to: { valueOf: () => 999999999 } as any,
  raw: { from: 'now-1h', to: 'now' },
} as TimeRange;

describe('transformToD3', () => {
  it('returns empty D3WideData for empty series', () => {
    const result = transformToD3({ series: [] } as unknown as PanelData, mockTimeRange);
  ...
  describe('wide frame input', () => {
    it('produces correct seriesNames from field names', () => {
      const result = transformToD3({ series: [wideDataFrame] } as unknown as PanelData, mockTimeRange);
  ...
  describe('multi-frame input', () => {
    it('merges frames into wide format with correct row count', () => {
      const result = transformToD3({ series: multiDataFrames } as unknown as PanelData, mockTimeRange);
  ...
  describe('sparse / null input', () => {
    it('produces rows for union of all timestamps', () => {
      const result = transformToD3({ series: sparseDataFrames } as unknown as PanelData, mockTimeRange);
      expect(result.rows).toHaveLength(3);
  ...
  it('handles single series gracefully', () => {
    const result = transformToD3(
      { series: [multiDataFrames[0]] } as unknown as PanelData,
      mockTimeRange
    );
  ...
  it('respects field.config.displayName over field.name', () => {
    ...
    const result = transformToD3({ series: [frame] } as unknown as PanelData, mockTimeRange);
